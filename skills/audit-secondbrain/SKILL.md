...
批次轉換調查報告時，系統會自動提取「查核意見」章節，依以下四層分類歸檔至 `資料處理/3.查核意見/`：

| 層級 | 分類依據 | 說明 |
|------|---------|------|
| **第一層** | 查核主題 | 依內容判斷：工程品質、採購程序、財務管理、內部控制、轉運站管理等 |
| **第二層** | 意見類型 | 建議事項、查明處理事項、注意事項 |
| **第三層** | 性質 | 洞察（發現潛在問題）、前瞻（預防性建議）、監督（追蹤改善） |
| **第四層** | 原始內容 | 查核意見標題 + 全文，末尾註記 `（截取自：[調查報告名稱]）` |

提取流程：定位 `# 查核意見` 章節 → 依 `**一、**` `**二、**` 等粗體編號逐項分割 → 每項獨立為一個 MD 檔 → 依上述四層結構分類歸檔。
...
> 115.08.26 更新：工作底稿生成原則已修訂，強調依據—事實—擬議三者呼應、縣府品質規定點項對應及扣點罰則計算，詳見 工作底稿生成原則1150826修訂版.md。
